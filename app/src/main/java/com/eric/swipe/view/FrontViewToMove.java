...
public class FrontViewToMove {
    ...
    private static int xToMove = 200;// 视图所要被移动的距离，默认200
    private ListView listView;// 如果所需移动的视图为ListView或其子类的item项，传入视图容器，限制其上下滚动
    private OnMoveChangeListener mMoveListener;
     ...
     */
    public FrontViewToMove(View frontView, View showView, int xToMove) {
        this.frontView = frontView;
        this.xToMove = xToMove;
        xToMove = showView.getWidth();
        moveListener();
    }
    ...
    public void setMoveChangeListener(OnMoveChangeListener listener){
                ...
                switch (MotionEventCompat.getActionMasked(motionEvent)) {
                    case MotionEvent.ACTION_DOWN: {
                        if(mMoveListener!=null){
                            if(mMoveListener.onMoved(FrontViewToMove.this)){
                                return true;
                            }
                        }
                    ...
                    case MotionEvent.ACTION_UP: {
                        ...
                        if (swap) {
                            if (!hasMoved) {
                                if(mMoveListener!=null){
                                    mMoveListener.onMoveChanged(FrontViewToMove.this);
                                }
    ...
    public void reinStatement() {
        int moveTo = 0;
        hasMoved=false;
        animate(frontView).translationX(moveTo).setDuration(10)
    ...
    public interface OnMoveChangeListener{
        void onMoveChanged(FrontViewToMove viewToMove);
        boolean onMoved(FrontViewToMove viewToMove);
        void onUpMotion();
    }
}
